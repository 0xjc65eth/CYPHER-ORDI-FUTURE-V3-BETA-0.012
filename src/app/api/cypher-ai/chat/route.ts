...
const SYSTEM_PROMPTS = {
  'pt-BR': `Você é CYPHER AI, uma assistente de trading de criptomoedas altamente avançada e especializada. Características principais:

🎯 ESPECIALIZAÇÃO:
- Expert em Bitcoin, Ethereum e mercado cripto
- Análise técnica avançada (SMC, ICT, Wyckoff)
...
⚠️ RESPONSABILIDADE:
- Sempre mencione que trading envolve riscos
- Não dê conselhos financeiros absolutos
- Eduque sobre gestão de risco
- Encoraje DYOR (Do Your Own Research)

Responda sempre em português brasileiro, de forma clara e objetiva. Use emojis relevantes para facilitar a leitura.`,
...
Entidades: ${JSON.stringify(entities || {})}`;
    ...
    if (!response) {
      const fallbackResponses = {
        'pt-BR': `🤖 **CYPHER AI** - *Modo Local*

Entendi sua solicitação sobre **${entities?.asset || 'o mercado'}**. 

📊 **Análise Disponível**:
• Posso ajudar com análise técnica
• Identificação de padrões SMC
• Gestão de risco
• Educação sobre trading

💡 **Como posso ajudar melhor?**
• "Analise o Bitcoin"
• "Mostre meu portfolio" 
• "Explique Smart Money Concepts"
• "Calcule risco para posição"

⚠️ *Lembre-se: Trading envolve riscos. Esta é uma análise educacional.*`,
