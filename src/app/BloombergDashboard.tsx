...
import { useMajorCryptoPrices, useBitcoinEcosystemPrices } from '@/hooks/useRealTimePrices';
import { TerminalLoading, SafeDataDisplay } from '@/components/ui/loading-states';
...
export default function BloombergDashboard() {
  ...
  const { prices: majorCryptoPrices, isLoading: cryptoLoading, lastUpdated: cryptoLastUpdated } = useMajorCryptoPrices(autoRefresh);
  const { prices: bitcoinEcosystemPrices, isLoading: ecosystemLoading } = useBitcoinEcosystemPrices(autoRefresh);

  // Merge real-time prices with market data
  const getRealTimeMarketData = (): BloombergMarketData[] => {
    const allPrices = new Map([...majorCryptoPrices, ...bitcoinEcosystemPrices]);
    
    // If we have real-time data, use it; otherwise fall back to API data
    if (allPrices.size > 0) {
      return Array.from(allPrices.entries()).map(([symbol, priceData]) => ({
        symbol,
        price: priceData.price,
        change24h: priceData.change24h,
        volume24h: priceData.volume24h,
        marketCap: priceData.marketCap,
        high24h: priceData.price * 1.05, // Approximate high
        low24h: priceData.price * 0.95,  // Approximate low
      }));
    }
    
    // Fallback to API-loaded data
    return marketData;
  };

  const currentMarketData = getRealTimeMarketData();
              ...
              <div className="flex-1 grid grid-cols-6 gap-0">
                <div className="p-3 border-r border-orange-500/30">
                  ...
                  <div className="text-sm font-bold text-green-400">
                    {currentMarketData.find(m => m.symbol === 'BTC')?.price.toLocaleString() || '105,847'} 
                    {(currentMarketData.find(m => m.symbol === 'BTC')?.change24h || 0) >= 0 ? '▲' : '▼'}{Math.abs(currentMarketData.find(m => m.symbol === 'BTC')?.change24h || 2.85).toFixed(2)}%
                  </div>
                </div>
                <div className="p-3 border-r border-orange-500/30">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-orange-500/60 font-mono ml-2">
                UPD: {(cryptoLastUpdated || lastUpdate || new Date()).toLocaleTimeString()}
              </span>
              {(cryptoLoading || ecosystemLoading) && (
                <span className="text-xs text-green-400 font-mono ml-2 animate-pulse">
                  • LIVE
                </span>
              )}
              {majorCryptoPrices.size > 0 && (
                <span className="text-xs text-green-400 font-mono ml-2">
                  📡 RT:{majorCryptoPrices.size + bitcoinEcosystemPrices.size}
                </span>
              )}
            </div>
          </div>
