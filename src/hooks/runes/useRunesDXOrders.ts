...
export function useRunesDXOrders(options: UseRunesDXOrdersOptions = {}) {
  ...
  // Get order fees estimation - FIXED: Cannot use hooks inside callbacks
  // This should be handled differently - fees should be calculated on the client
  const estimateOrderFees = useCallback(
    async (orderData: Omit<OrderFormData, 'slippageTolerance'>) => {
      if (!address || !orderData.amount || parseFloat(orderData.amount) <= 0) {
        return null;
      }
      
      try {
        return await runesDXService.estimateOrderFees({
          ...orderData,
          walletAddress: address,
        });
      } catch (error) {
