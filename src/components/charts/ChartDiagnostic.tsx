...
export function ChartDiagnostic() {
  ...
  useEffect(() => {
    const runDiagnostics = async () => {
      ...
      // Test 2: Check if Victory is available
      try {
        const victory = await import('victory');
        results.victoryImport = 'SUCCESS';
        results.victoryComponents = Object.keys(victory);
      } catch (error) {
        results.victoryImport = 'FAILED';
        results.victoryError = error instanceof Error ? error.message : 'Unknown error';
        errorList.push(`Victory import failed: ${results.victoryError}`);
      }

      // Test 3: Check if Chart.js is available
      try {
        const chartjs = await import('chart.js');
        results.chartjsImport = 'SUCCESS';
      } catch (error) {
        results.chartjsImport = 'FAILED';
        results.chartjsError = error instanceof Error ? error.message : 'Unknown error';
        errorList.push(`Chart.js import failed: ${results.chartjsError}`);
      }

      // Test 4: Check if Lightweight Charts is available
      try {
      ...
      // Test 5: Check client-side rendering
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-gray-800 p-4 rounded">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className={diagnostics.victoryImport === 'SUCCESS' ? 'text-green-400' : 'text-red-400'}>
              Victory: {diagnostics.victoryImport || 'Testing...'}
            </div>
            <div className={diagnostics.chartjsImport === 'SUCCESS' ? 'text-green-400' : 'text-red-400'}>
              Chart.js: {diagnostics.chartjsImport || 'Testing...'}
            </div>
