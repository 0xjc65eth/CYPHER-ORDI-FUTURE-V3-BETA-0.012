...
const TOKEN_DATA: ExtendedToken[] = [
  // Ethereum Tokens
  {
...
const TokenSelector: React.FC<TokenSelectorProps> = ({
  ...
  const filteredTokens = useMemo(() => {
    let tokens = TOKEN_DATA.filter(token => {
      // Filter by network
    ...
    return tokens
  }, [selectedNetwork.chainId, excludeToken, searchQuery, sortBy])
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400 mt-1">
                    {selectedNetwork.name} • {filteredTokens.length} tokens
                  </p>
                </div>
              </div>
              <Button
            ...
            <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)}>
              ...
              <div className="overflow-y-auto max-h-[400px] space-y-2">
                <AnimatePresence mode="wait">
                  {getTokensByCategory(activeTab).map((token, index) => (
                    <motion.div
                      key={token.address}
                ...
                {getTokensByCategory(activeTab).length === 0 && (
