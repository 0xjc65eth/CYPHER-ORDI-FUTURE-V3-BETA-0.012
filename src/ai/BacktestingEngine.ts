...
export class BacktestingEngine extends EventEmitter {
  ...
  private async runWalkForwardBacktest(
    ...
    while (currentStart + windowSize <= timestamps.length) {
      ...
      const inSampleResults = await this.runPeriodBacktest(
        strategy,
        inSampleData,
        timestamps[inSampleStart],
        timestamps[inSampleEnd - 1]
      );
      ...
      const outSampleResults = await this.runPeriodBacktest(
        strategy,
        outSampleData,
        timestamps[outSampleStart],
        timestamps[outSampleEnd - 1]
      );
      
      // Calculate efficiency
      const efficiency = this.calculateEfficiency(inSampleResults, outSampleResults);
  ...
  private async runMonteCarloSimulation(
    ...
    const returns = this.calculateDailyReturns(originalResults.equityCurve);
    const results = {
      returns: [] as number[],
    ...
    return {
      ...
      probabilityOfRuin: this.calculateProbabilityOfRuin(results.returns),
  ...
  private applyRiskManagement(marketData: any, timestamp: Date): void {
    ...
    for (const [id, position] of this.positions) {
      this.updateStopLoss(position, marketData, timestamp);
    }
  }
  ...
  private shouldExecuteTrade(signal: any): boolean {
    ...
    if (this.positions.length >= this.config.constraints.maxOpenPositions) {
      return false;
    }
  ...
  private combineWalkForwardResults(results: any[]): any {
    ...
    combined.sharpeRatio = this.calculateSharpeRatio();
