...
// Função para obter preços de exchanges (simulado para demo)
async function getExchangeQuotes(
  ...
  // Simular chamadas para APIs reais das exchanges
  for (const exchange of exchanges) {
    try {
      // Em produção, fazer chamadas reais para as APIs
      const mockQuote = await simulateExchangeQuote(exchange, fromToken, toToken, amount, network);
      quotes.push(mockQuote);
    } catch (error) {
      console.error(`Erro ao obter cotação de ${exchange.name}:`, error);
    }
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const quotes = await getExchangeQuotes(fromToken, toToken, amount, network);
